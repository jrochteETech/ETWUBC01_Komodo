...
def apply_decision(decision, current, total, dry_run, provider):
    ...
    try:
        ...
        if action == "keep":
            msg = u"No changes made."
            print(u"  [OK] {}".format(msg))
            set_result(u"OK", msg)
            return True
            ...
            print(u"  [OK] {}".format(msg))
            set_result(u"OK" if not dry_run else u"DRY RUN", msg)
            return True
            ...
            if not dry_run:
                ...
                parent = tag_path.rsplit("/", 1)[0] if "/" in tag_path else "[default]"
                system.tag.configure(parent, [tag_config], "o")
            msg = u"Priority set to '{}'{}.".format(new_pri, u" (dry run)" if dry_run else u"")
            print(u"  [OK] {}".format(msg))
            set_result(u"OK" if not dry_run else u"DRY RUN", msg)
            return True
...
def write_results(output_path, all_rows):
    fieldnames = ["path", "alarmName", "action", "newPriority", "status", "message"]
...
def main():
    ...
    for current, decision in enumerate(decisions, start=1):
        if apply_decision(decision, current, total, DRY_RUN, PROVIDER):
            success += 1
        else:
            failed += 1
