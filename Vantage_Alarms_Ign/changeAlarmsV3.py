...
def apply_decision(decision, current, total, dry_run, provider):
    ...
    return success, customEmailRemoved, labelChanged
...
def write_results(output_path, all_rows):
    fieldnames = ["path", "alarmName", "previousPriority", "action", "newPriority", "status", "message", "customEmailChanged", "labelChanged", "currentLabel", "newLabel"]
...
def main():
    ...
    for current, decision in enumerate(decisions, start=1):
        decision_success, custom_email_changed, label_changed = apply_decision(decision, current, total, DRY_RUN, PROVIDER)
